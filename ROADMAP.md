...
- **v10.23.2** **The guided `/smp` existed and was unreachable.** Two `/smp` dispatchers in one file; v10.23.0's fix went into `OTRv4IRCClient.handle_command`, which `EnhancedOTRv4IRCClient` — the only class instantiated — shadows for that command. A handset typing `/smp` got `Usage: /smp <command> [args]`. Bare `/smp` and `/smp start` now both ask for the passphrase hidden and verify in one step; `/smp <secret>` verifies immediately and says it was echoed; the dead dispatcher is deleted. The unit tests passed throughout because they called `_smp_verify` on a stub and never went through `handle_command` — 27 new tests do, 8 mutations killed.
- **v10.23.1** **DAKE1 was displayed twice, not sent twice.** `_switch_panel` replays a tab's buffer on entry and had no guard against being asked to enter the tab already active, so an inbound DAKE2 fragment reprinted the handshake lines under their original timestamps. Guard added in the callee (fifteen call sites, three of which checked), with `force=True` for the one caller — `/switch` — where the redraw is the request. 16 tests, 5 mutations killed.
- **v10.23.0** **The IRC client finally has the guided SMP flow.** It was promising a passphrase prompt it did not have: `_finish_trust` said "it will ask for the passphrase", and `/smp` answered "type it, it will be visible on this terminal" — while `set_input_mask()` had existed since v10.15 with no caller anywhere in the file. Now first contact pins silently (a change is reported and never auto-accepted), bare `/smp` arms a masked read, and a peer's held SMP1 gets the user asked with `y` resuming it rather than restarting. It imports the XMPP client's `SmpFlow` rather than reimplementing it, because INV-06 is structural there — no edge from a remote transition into AWAITING_SECRET. Also ships the INV-06 coverage `SMP_UX_AUDIT.md` asked for in item 4 and never got: `otrv4+.py`'s inbound call graph is now walked too. 53 tests, 7 mutations killed.
- **v10.22.0** **The chat prefix now says what protected the message.** A verified session showed `[otr] <bob> ohhh lala` with `[otr]` green and nothing else — the reassurance was a colour and only a colour. Now 🔐 blue for encrypted-and-SMP-verified, 🔒 yellow for encrypted-but-unverified: two padlocks rather than one, because the same padlock on both would give a reader who never ran SMP exactly the reassurance of one who did. They differ by glyph *and* colour, since emoji are small on a handset and colour is invisible to some readers. The old green also contradicted the project's own table, where green is FINGERPRINT — pinned but not SMP-verified. Both glyphs were already in `_LOG_MARKERS`, checked before the change: one that was not would have stopped message-body redaction and written every received message to the session log in plaintext. 18 tests, 6 mutations killed.
- **v10.21.1** **The payment URI was encoded in the QR and shown nowhere.** Caught on review of the release notes: they said the QR was built from the `monero:` URI but never said the URI was displayed, because it was not. A reader whose wallet could not scan fell back to the bare address and silently lost the amount. All three routes are now on screen — scan the QR, copy the address, copy the payment URI — and the copyable text moved *below* the QR, since a 22-row symbol pushed the address off the top of a handset terminal. 10 tests, 5 mutations killed, one only after the ordering test turned out to match the address inside the URI line.
- **v10.21.0** **`/tip`: relay a Monero address, and only an address.** Ask a verified peer where to send a tip, get it back as text and a scannable QR, pay from your own wallet — the client sends no money and builds no transaction. The specification's interactive prompt was deliberately not implemented: a remote message arming a capture of the user's next typed line is the mechanism INV-06 forbids, and here that line would be transmitted rather than merely stored, so an unanswerable request is reported and answered with `/setxmr` then `/tipreply`. A parametrised test caught a real hole on the way: Python's `\d` is Unicode-aware, so Arabic-Indic digits passed the amount check and would have gone into a `monero:` URI. New engine hook for TLV `0x0020`, narrowed to a one-element allowlist so it cannot become a general extension point. 103 tests, 6 mutations killed — two only after the tests written for them turned out unable to fail.
- **v10.20.0** **A courier for multisig coordination, and no Monero code at all.** The audit recommended against putting Monero's cryptography in the Rust core — yanked crate, FROST multisig that cannot interoperate with `monero-wallet-cli`, an experimental feature that is off by default, and a hard fork that removes CLSAG. So the client relays opaque base64 between two wallets the users run themselves and holds no Monero code: no Ed25519, no FROST, no consensus format, no MSRV bump. `/trade init|accept|decline|blob|confirm|cancel` over `?OTRv4-TRADE:` inside the established session. Two new invariants: it is not a wallet (import list asserted exactly, identifiers walked for anything key- or network-shaped) and no trade advances on an unverified or changed peer (checked every message, both directions, fail-closed). No arbitration service and no arbitrator key. 86 tests, 6 mutations killed. Not yet carried a real blob between two wallets — that is the stagenet runbook in TRADE.md.
- **v10.19.0** **PyO3 0.29 for GHSA-36hh-v3qg-5jq4, and IRC scrollback stops outliving the connection.** The advisory (`nth`/`nth_back` computing `index + n` before the bounds check, then reading with `get_item_unchecked`) was audited before it was patched: the crate's whole Python-facing surface is bytes, str, integers and opaque handles, `PyList`/`PyTuple` appear nowhere, so the vulnerable path is unreachable — informational here, upgraded to 0.29.2 regardless, with one API rename at two sites and four transitive dependencies removed. On the IRC side, a reconnect used to replay the whole previous conversation into the new session under a nick the client had been forced to rename itself into, three times in one evening. Three causes: unbounded never-cleared history, a 5s backoff shorter than any ping timeout, and a permanent rename on 433. Now capped at 1000, purged at every connection boundary including process exit, 30/60/90/120s backoff, and up to four attempts to reclaim the original nick. Timestamps carry the date. `pip install socks` no longer looks like the fix for a missing PySocks. 117 tests, 7 mutations killed, 1 equivalent.
- **v10.18.6** **Building a Python extension with no `.so` is now an error.** v10.18.5 warned; a second Alpine report showed a warning scrolls past looking exactly like rustc's own `dropping unsupported crate type` line, which had already gone unread twice. With `--features extension-module` it fails outright and names both fixes; without it, `cargo test` still only warns. `cargo test` on Alpine needing the same flag is documented (`cannot find -lpython3.12`). And a test that would have passed on a broken message was fixed: compiled standalone at an old edition, inline format captures rendered as the literal `{fix}`. 3 tests, 2 mutations killed, 1 correctly survived.
- **v10.18.5** **And on musl it now produces a `.so`.** v10.18.4 got the build to compile; it then failed more quietly, by succeeding. musl enables `crt-static` by default and rustc cannot build a cdylib against a static C runtime, so it drops the crate type, says so in one line among hundreds, and exits 0 — leaving an `.rlib` and no `.so`, and a `cp` that fails a command later for no visible reason. `crt-static` is now off for the musl triples, and `build.rs` warns when it is on so a bypassed config is not silent. 10 tests, 3 mutations killed.
- **v10.18.4** **The Rust core builds on musl.** On Alpine the build died before compiling any Rust: PQClean's `compat.h` uses `__GNUC_PREREQ`, a glibc macro musl does not define, so the preprocessor met an undefined identifier followed by `(`. Upstream has no fixed release. `Rust/.cargo/config.toml` supplies the macro for the musl triples only — keyed per-target so glibc builds never see it, space-free because cc-rs splits CFLAGS on whitespace, and verified equivalent to glibc's own definition by compiling both against each other. 11 tests, 5 mutations killed, one of which survived the first attempt.
- **v10.18.3** **A wrong `--peer` now looks like a wrong `--peer`.** A device run lost `.i2p` off the end of `--peer`. The address was well-formed, so validation passed; the server bounced the outgoing DAKE with one unexplained line, the peer's own DAKE established a session under their real JID, and `/smp` then reported "no encrypted session ... Run /otr first" about an address that could never have one — while a fully verified session sat under the real JID. Every line was individually correct and together they hid the cause; both people concluded the other end was broken. Now flagged at startup with the corrected command to type, explained when the server refuses the domain, and every no-session message names the session that does exist. 15 tests, 3 mutations killed.
- **v10.18.2** **The Rust core builds silent, and `Cargo.toml` is valid TOML.** `cargo build` emitted 10 warnings; it now emits none, nor does clippy or `cargo test`. The one that mattered was not a warning at all: `Cargo.toml` held a multi-line inline table, which TOML 1.0 forbids. Cargo accepts it as an extension so the build was green, while `maturin build` failed before compiling anything — verified against maturin 1.7.8 and 1.8.1. Nine deprecated `Nonce::from_slice` calls became `Nonce::from`, and the two taking a slice of peer- or disk-supplied bytes now refuse rather than panic. `Dakeresult::success` gated to match its `legacy-dake-keys` callers instead of silenced. 111 Rust tests, 5 new manifest tests, 1 mutation killed.
- **v10.18.1** **`/otr` can actually unstick a hung handshake.** `EnhancedSessionManager.end_session` did not exist, and three call sites had been calling it since they were written — clearing a peer, DAKE glare teardown, and the `/otr` force reset. All three raised AttributeError into an `except` that printed it and carried on, so none ever tore anything down. `terminate_session` could not stand in: it leaves the entry in `self.sessions`, so the next lookup returns the dead session and the handshake stays stuck. Worse, the reset's client-side cleanup shared the engine's `try`, so one exception on the first line skipped all of it. Now a local teardown with no DISCONNECTED TLV and no trust-database change, and two separate steps. 16 tests, 3 mutations killed. Turns three inert paths live, glare among them — wants a two-handset re-test.
- **v10.18.0** **A transfer says what it is doing.** A completed transfer printed nothing: `on_done` returned the final path and `handle_control` discards a handler's return value, so the file landed on disk silently and success was indistinguishable from a stall. And `render_progress`, which had existed with tests since v10.14.0, was never called by the client. Both directions now show a bar, percentage, bytes done of total, rate and ETA; both ends announce completion. The clock starts at accept rather than at the offer, no ETA is extrapolated from a single instant, and lines are throttled to one a second with the final one never dropped. 24 tests, 4 mutations killed.
- **v10.17.2** **NOTICE, generated from the dependency graph.** The attribution file v10.17.0 flagged as missing. `tools/generate_notice.py` walks normal dependency edges from the root — 109 of 137 resolved packages actually ship — and emits every crate with its version, source, copyright holders read from its own licence files, and both the licence it offered and the one taken. 104 MIT, 4 BSD-3-Clause, 1 Apache-2.0 with LLVM exception, 1 Unicode-3.0; only taken texts are reproduced, so `r-efi`'s declined LGPL option does not appear. Two generator defects caught first: Apache-2.0 body prose matched as a copyright, and `(MIT OR Apache-2.0) AND Unicode-3.0` split on OR. 19 tests, 2 mutations killed.
- **v10.17.1** **An accepted transfer is not rate-limited at all.** v10.16.2 raised the receiver's budget; a 1.9 MB device test walked off the new cliff at chunk 33 of 119. Raising a limit only moves a cliff — a receiver whose transfers survive only while the sender paces itself is broken by construction. An accepted transfer is now exempt, bounded by the quantity the user agreed to receive rather than by a rate. Separately, abandoning a transfer never told the sender, which pushed ~430 more stanzas into a throttled peer and starved the keepalive until the session died: it now sends CANCEL first. Straggler chunks and repeated throttling are counted, not printed once each. 13 tests, 3 mutations killed.
- **v10.17.0** **Dual-licensed: AGPL-3.0, or a commercial licence.** The licensing audit was re-run first and settled the question it had left open: every dependency is permissive — 137 Rust packages, the Python layer, Chaquopy (MIT) and i2pd (BSD-3) — so the only copyleft was the project's own, which is the condition dual licensing needs. AGPL rather than GPL because the GPL's obligation attaches to distribution, so a hosted service triggers nothing; §13 closes that. Not a non-commercial licence, because that would cost F-Droid distribution, and for a security tool being genuinely redistributable is worth more than closing the comply-instead-of-pay route. GPL-3.0 releases up to v10.16.2 keep their rights permanently. A CLA and contractor IP-assignment terms keep the commercial half grantable. 17 tests. Outstanding: no NOTICE file, and the copyrightability of AI-generated code needs advice before any licence is sold.
- **v10.16.2** **`/sendfile` actually transfers.** The first real two-phone transfer failed three ways at once. The receiver's rate limiter (20 messages / 5 s) dropped four fifths of a 340 KB file as if it were a flood, and since the chunk AEAD is a sequence the first gap ended the transfer — `chunk 10 arrived out of order` was the symptom, not the fault. Nothing paced the sender. And the whole file was sent inside the inbound message handler, on the event loop, so keepalives could not run and the transfer took the connection down with it. An accepted transfer from an SMP-verified peer now raises that peer's budget for as long as it runs (raised, not lifted; an offer alone buys nothing), the sender paces on the stanzas it actually sent, and `pump_step` sends one chunk per turn from a task rather than blocking the loop. A lost chunk abandons the transfer with one message instead of twenty. 20 tests, 3 mutations killed.
...
- **v10.14.0** **`/sendfile` — encrypted file transfer over XMPP.** A fresh AES-256-GCM FileKey per transfer, generated, used and destroyed inside Rust with no getter; wrapped under a key derived from the session's DAKE extra symmetric key, which is the value OTRv4 reserves for out-of-band use and the one session secret that does not advance as the ratchet moves. No second ML-KEM exchange: the brace key already folds ML-KEM-1024, so the session is already post-quantum and already authenticated. 64 KiB chunks with the index and a final flag in the AAD, so reordering, duplication and truncation all fail the tag. Six verification steps before an atomic rename into `~/.otrv4plus/files/`. XMPP only, enforced by test. Python 2283 passed, Rust 101 passed. Not tested on a device at the time — four device-found defects followed in v10.16.1, v10.16.2, v10.17.1 and v10.18.0 before a transfer completed between two handsets on 2026-09-05.
