...
| **Android app** | `android/app/build.gradle.kts` | `0.7.0-experimental.rc.3+core.<crate>` | An APK at an earlier maturity than the Termux client. Its own track, with the client version it embeds recorded as semver build metadata. |
...
**The Android track's changes are recorded in `CHANGELOG.md` under
`## Android <version>` headings** (since 0.6.0), so they cannot be mistaken for
a `v10.x` client release. The open gate list stays in `ANDROID_XMPP_MILESTONE.md`
§7 and `ANDROID_CALL_AND_FILE_DEVICE_TEST.md`, and the published prerelease's
notes are generated by `.github/scripts/publish-experimental-apk.sh` and held to
that list by `tests/test_release_notes.py`.

### Release candidates

`-experimental.rc.N` marks a build in which **every repository-level gate is
closed** — nothing known is missing from the code, the dependencies, the
licensing or the documentation — and only the handset checklist remains. It
keeps `-experimental` because that checklist has not been run; `.rc.N` says
nothing else is outstanding. A candidate is published under its own kept tag
(`android-v<version>`), not the rolling `android-experimental`. When the
checklist passes on a candidate, the next build drops both suffixes.
