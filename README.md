...
| **Native Android app** | **Under active development.** Sign-in, the roster, contacts, presence, 1:1 conversations and rooms have been driven on a handset against the live server. **OTR itself has not been observed completing on a device**, and voice on the APK has never run — so nothing sent from it should be assumed encrypted beyond the I2P tunnel. Open gates, item by item: [ANDROID_XMPP_MILESTONE.md](ANDROID_XMPP_MILESTONE.md) §7; root causes and history: [ANDROID_FOUNDATION_REPORT.md](ANDROID_FOUNDATION_REPORT.md) |
...
- [VOICE_PATH_INVESTIGATION.md](VOICE_PATH_INVESTIGATION.md) — what the voice diagnostics were actually measuring, and the two that were wrong
