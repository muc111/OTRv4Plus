...
Three-message handshake per OTRv4 §4.2-4.3. X448 ephemeral DH + ML-KEM-1024 encapsulation. Both peers contribute entropy. **Now fully inside Rust**: key generation, DH exchanges, KEM encap/decap, MAC calculation, and ring signature verification happen without Python touching any secret. The Python layer only sees opaque byte buffers and final session keys.
...
| DAKE secrets | Rust heap/stack | Nothing | ZeroizeOnDrop |
...
| Ed448 private keys | OpenSSL C heap (C extension) | Protected by pyo3 | OPENSSL_cleanse |

On /quit: all Rust structures drop (Zeroize), C buffers cleansed, ~/.otrv4plus destroyed per NIST SP 800-88r1.
...
- **No formal audit.** 313 automated tests. No third-party review. No ProVerif/TLA+ models.
...
|----------------|----------|---------------------|---------------|
| Double Ratchet  | Rust     | ZeroizeOnDrop ✅    | Python + Rust (future) |
| DAKE engine     | Rust     | ZeroizeOnDrop ✅    | Python only currently |
| SMP engine      | Rust     | ZeroizeOnDrop ✅    | Python only currently |
| SMP vault       | Rust     | ZeroizeOnDrop ✅    | Python only currently |
| Ring signatures | C ext    | OPENSSL_cleanse     | Python           |
| ML-KEM-1024     | C ext    | OPENSSL_cleanse     | Python           |
| ML-DSA-87       | C ext    | OPENSSL_cleanse     | Python           |
| Identity keys   | C ext    | OPENSSL_cleanse     | Python           |
