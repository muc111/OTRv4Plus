...
| **Double Ratchet** | All chain key advancement, message key derivation, AES‑256‑GCM encrypt/decrypt, skipped key management, and replay detection run inside the Rust `otrv4_core` crate. Keys are `Zeroize`‑on‑drop – no Python memory exposure. |
...
**Memory safety note:** SMP exponents are generated by a C extension (`_ct_rand_range` → Python `int`) and used in ZKP computation in Python. After use they are stored in the Rust vault (`RustSMPVault`) and zeroized on session end. Full Rust‑only SMP (no Python `int` intermediates) is planned – see section 8.
...
313 tests (299 Python + 14 Rust) covering:
...
8. Rust Migration Status & Memory Safety

The cryptographic core is being progressively moved into Rust so that all secret key material benefits from deterministic Zeroize and mlock protection, rather than depending on Python’s garbage collector. The current status is as follows:

Component Secret storage Zeroization
Double ratchet Rust otrv4_core with Zeroize Deterministic on drop / zeroize() – no Python exposure
SMP exponents Python int during ZKP computation → Rust vault (RustSMPVault) afterwards Vault zeroized on session end; Python int is GC‑dependent
DAKE DH secrets Python bytes during KDF Cleansed by _ossl.cleanse after use
Identity keys (Ed448/X448) Python OpenSSL objects (cryptography) No zeroization (OpenSSL heap)

Migration roadmap:

Phase What moves to Rust Status
1. Double ratchet Chain keys, message keys, AES‑256‑GCM, skip keys, replay cache ✅ Done (v10.5.8)
2. SMP engine Full state machine: exponent generation, ZKP computation, secret hashing – no Python int intermediates 🔜 Planned
3. DAKE key derivation DH shared‑secret handling, KDF‑1 domain‑separated key derivation 🔜 Planned
4. Long‑term keys Ed448 & X448 private keys stored in Rust SecretVec, signing/exchange via Rust bindings 🔜 Planned
5. Hardening mlock all Rust secret buffers, force‑zeroize on panic, formal audit of Rust↔Python boundary 🔜 Planned

The double ratchet is the most critical component – it protects every chat message after the handshake. Its migration to Rust is complete. The remaining phases will eliminate all Python‑side secret handling, closing the last microsecond‑scale windows of exposure. See ROADMAP.md for details on timelines and technical approach.
