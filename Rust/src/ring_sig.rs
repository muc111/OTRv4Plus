...
//!     t1   = SHAKE256(seed ‖ 0x01, 57) mod Q   (ephemeral)
//!     T1   = t1·G
...
/// Derive the ephemeral scalar `t1` deterministically from the seed.
/// t1 = SHAKE256(seed ‖ 0x01, 57) mod Q.
/// Using a distinct usage tag (0x01) ensures t1 != a1.
fn derive_ephemeral_scalar(seed: &[u8]) -> Scalar {
    debug_assert_eq!(seed.len(), 57);
    let mut h = [0u8; 57];
    shake256_multi(&[seed, &[0x01u8]], &mut h);
    let s = scalar_from_le57_mod_q(&h);
    h.zeroize();
    s
}
...
pub fn ring_sign_bytes(
    ...
    // ── Step 2: ephemeral scalar t1 ────────────────────────────────────────
    let t1 = derive_ephemeral_scalar(seed);
...
mod tests {
    ...
    /// Determinism: same inputs → t1 derives deterministically → but c2/r2 are
    /// random, so signatures differ.  However t1 must be deterministic.
    #[test]
    fn t1_is_deterministic() {
        let seed = [0x99u8; 57];
        let t1_a = derive_ephemeral_scalar(&seed);
        let t1_b = derive_ephemeral_scalar(&seed);
        assert_eq!(scalar_to_le57(&t1_a), scalar_to_le57(&t1_b));
    }
}
