...
The C extensions (`otr4_crypto_ext`, `otr4_ed448_ct`, `otr4_mldsa_ext`) load at startup and **remain in use throughout production code**. `otr4_crypto_ext` provides constant-time big-number arithmetic for SMP, memory wiping (`cleanse`), `mlock`, and an alternate ML-KEM-1024 keygen path used by the legacy `MLKEM1024BraceKEM` Python class. `otr4_mldsa_ext` provides ML-DSA-87 keygen, sign, and verify. The Python `cryptography` library is also imported in production for Ed448 public-key wrapping (UI-side fingerprint display) and AES-GCM (persistent SMP-secrets store). Replacing these with pure-Rust equivalents is the subject of phases 5.3f through 5.3j on the roadmap and is multi-commit work.
...
[OTR:DAKE]      STATE: IDLE -> RECEIVED_DAKE1
[OTR:DAKE]      STATE: SENT_DAKE2 -> ESTABLISHED | DAKE3 verified
                  -- hybrid (ring-sig ok + ML-DSA-87 ok)
[OTR:peer]      RATCHET: None -> ACTIVE | ratchet: Rust (Phase-4 opaque
                  handle; keys never in Python)
```
...
5. **C extensions and the cryptography library are still load-bearing in production.** `otr4_crypto_ext` provides constant-time SMP big-number arithmetic, memory wiping, and an alternate ML-KEM path. `otr4_mldsa_ext` provides ML-DSA-87 primitives. The `cryptography` library is used for Ed448 public-key wrapping (UI-side) and AES-GCM (persistent SMP secrets). Dropping these is a multi-phase project (see ROADMAP).
...
7. v10.6.16 migrated the post-quantum KEM from `pqcrypto-kyber 0.8` (round-3 Kyber) to `pqcrypto-mlkem 0.1.1` (FIPS 203 ML-KEM-1024). v10.6.14 removed `lazy_static`. `pqcrypto-mldsa` remains at 0.1.2 (latest published).
