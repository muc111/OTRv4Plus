...
| 6 | ~~Channel / message logs (`~/.otrv4plus/logs/channels/*.enc`)~~ | ~~files~~ | **RESOLVED at v10.13.1 — the files, the key and the cipher are all gone.** `otrv4plus_log.py` is now an in-memory ring that dies with the process: no disk, no key, no cryptography. The hand-rolled AEAD (SHAKE-256 keystream XOR + HMAC-SHA3-512 truncated to 16 bytes) and its plaintext key file `~/.otrv4plus/channel_log.key` were deleted rather than re-based onto the Rust AES-256-GCM, because scrollback that never persists has nothing to protect at rest. Pinned by `tests/test_no_parallel_crypto.py`. | — | — | none — closed | — |
