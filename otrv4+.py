...
class RustDAKEAdapter :
    ...
    def process_dake1 (self ,dake1_msg :str ,peer_key :str ="unknown")->bool :
        ...
        try :
            ...
            self ._raw_dake1_bytes =raw 
            self ._log_debug (f"process_dake1: raw={len (raw )} bytes, first4={raw [:4 ].hex () if len (raw )>=4 else raw .hex ()}")
...
class Pager :
    ...
    def display (self ,lines :List [str ],header :str ="",footer :str =""):
        ...
        try :
            ...
            global _input_pos 
            _input_buffer .clear ();_input_pos =0 
...
class Screen :
    ...
    def _body_visual_lines (self )->list :
        ...
        for entry in panel .history :
            ts =colorize (time .strftime ("%H:%M:%S",
...
class EnhancedSessionManager :
    ...
    def _handle_dake1 (self ,peer :str ,dake1_msg :str )->Optional [bytes ]:
        ...
        with self .lock :
            ...
            if session .session_state not in (SessionState .PLAINTEXT ,) and session .session_state .name not in ('PLAINTEXT','FAILED','FINISHED'):
                self .tracer .trace (peer ,"ERROR","DAKE1","REJECTED",f"session in state: {session .session_state .name }")
                return None 
            if session .session_state .name in ('FAILED','FINISHED') or session .dake_engine is not None :
                session .session_state =SessionState .PLAINTEXT 
                if hasattr (session ,'dake_state'): session .dake_state =DAKEState .IDLE 
    ...
    chunk :str ,now :float )->Optional [str ]:
        ...
        if idx not in state ['parts']:
            ...
            if DEBUG_MODE :
                    ...
                    if _c :_c ._emit ("debug",f"[OTRFragment] {idx }/{total } for {sender }")
                    else :_emit_line (f"[OTRFragment] {idx }/{total } for {sender }")
        ...
        if len (state ['parts'])==total :
            ...
            if DEBUG_MODE :
                    _c =getattr (__import__ ("builtins"),"_active_client",None )
                    if _c :_c ._emit ("debug",f"[OTRFragment] reassembled {len (combined )} chars from {sender }")
                    else :_emit_line (f"[OTRFragment] reassembled {len (combined )} chars from {sender }")

            try :
                _cr =getattr (__import__ ("builtins"),"_active_client",None )
                if _cr and _cr ._prompt_refresh_cb :_cr ._prompt_refresh_cb ()
            except Exception :
                pass 
...
class OTRMessageFragmenter :
    ...
    receiver_tag :int =0 )->List [str ]:
        ...
        if DEBUG_MODE :
            ...
            if _c :_c ._emit ("debug",_msg )
            else :_emit_line (_msg )
...
class OTRv4IRCClient :
        ...
        for i ,frag in enumerate (fragments ):
            ...
            if len (fragments )>1 :
                try :
                    ...
                    _set_prompt (f"\x1b[36m↑ TX {target } [{i +1 }/{len (fragments )}] {_tb }\x1b[0m ")
                except Exception :
                    pass 
            if not self .send (f"PRIVMSG {target } :{frag }"):
                ...
                if len (fragments )>1 :
                    ...
                    colorize (f"⚠ Fragment send failed at {i +1 }/{len (fragments )} — /otr {target } to retry","yellow"))
                break 
        if ok :
            self ._last_otr_sent [target ]=time .time ()
        if len (fragments )>1 :
            try :
                if self ._prompt_refresh_cb :self ._prompt_refresh_cb ()
            except Exception :
                pass 
        return ok 
            ...
            if command =="PART":
                ...
                if sender ==self .nick :
                    ...
                    if channel in self .panel_manager .panels :
                        ...
                        if channel in self .panel_manager .panel_order :self .panel_manager .panel_order .remove (channel )
                        if _was :self ._switch_panel ("system")
                        elif getattr (self ,"_tui_enabled",False )and self ._screen :self ._screen .redraw_full ()
    ...
    def handle_numeric_reply (self ,code :int ,params :List [str ],trailing :Optional [str ]):
        ...
        try :
            ...
            if code ==352 :
                ...
                if _who_real and _who_real .startswith ("0 "):_who_real =_who_real [2 :]
            ...
            if code ==314 :
                ...
                self .add_message (_wp4 ,f"  {colorize_username (_ww_nick )}  {_sanitise (_ww_user +'@'+_ww_host ,80 )}  {_sanitise (_ww_real ,60 )}")
                return 
            if code ==315 :
                if getattr (self ,"_who_pending",False ):
            ...
            if code ==366 :
                ...
                if getattr (self ,'_pending_names_pager',None )==channel :
                    ...
                    _plines =[]
                    if _otr_count :
                        _plines .append (colorize (f"  🔒 = OTRv4+ ({_otr_count} user(s)) — /otr <nick> to encrypt","blue"))
                    def _grp (label ,users ,color ):
                        if not users :return 
                        _plines .append (colorize (f"  {label } ({len (users )}):",color ))
                        col_width =20 ;cols =max (1 ,56 //col_width );row =[]
                        for pfx ,nk in users :
                            disp =f"{pfx }{nk }"
                            entry =(colorize (f"  🔒{disp :<{col_width -1 }}","blue")
                            if _otrv4_map .get (nk ) else colorize (f"  {disp :<{col_width }}",color ))
                            row .append (entry )
                            if len (row )>=cols :_plines .append ("".join (row ));row =[]
                        if row :_plines .append ("".join (row ))
            ...
            if code ==312 :
                ...
                info =trailing or ""
                self .add_message (getattr (self ,"_whois_panel","system"),
            ...
            def _on_first_fragment (s :str ,n :int ,chunk :str ="")->None :
                ...
                try :
                    ...
                    if _should_switch :
                        ...
                        _in_channel =(
                        _cur is None 
                        or (_cur_panel is not None 
                        and _cur_panel .type in ("channel","system"))
                        or _cur ==s 
                        )
    ...
    def _display_plaintext (self ,sender :str ,target :str ,message :str ):
        ...
        if message .startswith ("\x01ACTION ")and message .endswith ("\x01"):
            ...
            self .add_message (panel ,colorize (f"* {sender } {_sanitise (message [8 :-1 ],490 )}","cyan"))
            return 
        panel =target if target .startswith ("#")else sender 
        ...
        if cmd =="help":
            ...
            if want :
                ...
                with _print_lock :
                    ...
                    sys .stdout .flush ()
                self .add_message ("system",colorize (
                "TUI off — back to scrollback mode.","dim"))
                if self ._prompt_refresh_cb is not None :
                    self ._prompt_refresh_cb ()
            ...
            if _t and not _t .startswith ("#"):_t ="#"+_t 
            ...
            if _cn :self .send (f"PRIVMSG {_cn } :\x01{_cc }{_ca }\x01")
        elif cmd =="cycle":
            _cy =parts [1 ]if len (parts )>1 else self .panel_manager .active_panel or ""
            if _cy and not _cy .startswith ("#"):_cy ="#"+_cy 
            if _cy :
            ...
            _ch2 =self .panel_manager .active_panel or ""
            if _ch2 .startswith ("#"):self .send (f"MODE {_ch2 } +b {_sanitise (parts [1 ],128 )}")
            ...
            _mop =parts [1 ]
            if _mop =="l":self .send ("MONITOR L")
            elif len (parts )>2 :self .send (f"MONITOR {_mop .upper ()} {','.join (_sanitise (p2 ,64 )for p2 in parts [2 :])}")
            ...
            try :_sp =int (parts [2 ])if len (parts )>2 else self .config .port 
            except ValueError :_sp =self .config .port 
            if _sv :
                self .config .server =_sv ;self .config .port =_sp ;self .server =_sv 
    ...
    def show_help (self ,topic =None ):
        if topic :topic =topic .lower ().strip ()
        lines =[]
        header ="OTRv4+ Help"
...
def main ():
    ...
    import builtins as _b ;_b ._active_client =client 
