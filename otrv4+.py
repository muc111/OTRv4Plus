# =============================================================================
#  OTRv4+  -  IRC transport client
#  Post-quantum Off-the-Record messaging over IRC (clearnet TLS / Tor / I2P).
#
#  SECURITY MODEL (enforced throughout this file):
#    * All cryptography lives in the Rust core (otrv4_core). This Python layer
#      never holds private key material and never implements a primitive; it
#      orchestrates the handshake, ratchet, and SMP and renders the UI.
#    * Every byte of untrusted network data (nicks, channels, topics, message
#      bodies, AND decrypted OTR payloads) passes through _sanitise() before it
#      reaches the terminal, blocking ANSI/OSC/CSI escape-sequence injection and
#      OSC title-hijack from a malicious server or peer.
#    * Inbound OTR fragments are bounded on every axis (per-sender count, total
#      senders, in-flight fragments, 1 MiB reassembly cap, and a timeout) to
#      prevent memory-exhaustion denial of service. See OTRFragmentBuffer.
#    * TLS certificate validation is ON by default. It is only relaxed behind an
#      explicit opt-in, for self-signed servers reached over an already
#      cryptographically authenticated I2P or Tor tunnel.
#    * Subprocess calls (termux-notification) use argv lists, never a shell, and
#      untrusted values are sanitised against command-line option injection.
#    * Fingerprints are pinned on first use (TOFU); a mismatch raises and aborts.
#
...
class RingSignature:
    ...
    def sign(cls, handle, A1_bytes: bytes, A2_bytes: bytes, msg: bytes) -> bytes:
        ...
        v10.6.12 (Phase 5.3e): the signing key is now passed as an
        Ed448KeyHandle (Rust-owned).  The handle's .ring_sign() method
...
class OTRv4TLV:
    ...
    def decode_one(cls, data: bytes, offset: int = 0) -> Tuple["OTRv4TLV", int]:
        ...
        if offset + 4 > len(data):
            raise ValueError(
                f"TLV header truncated at offset {offset } " f"(need 4, have {len (data )-offset })"
            )
...
class OTRv4DataMessage:
    ...
    def decode(cls, data: bytes) -> "OTRv4DataMessage":
        ...
        try:
            ...
            if mtype != cls.TYPE:
                raise ValueError(
                    f"Wrong message type: 0x{mtype :02x} (expected 0x{cls .TYPE :02x})"
                )
        ...
        if b2 in (b"[", b"O"):
            ...
            with _print_lock:
                ...
                if seq == b"D":
                    ...
                    if _input_pos > 0:
                        sys.stdout.write(f"\x1b[{_input_pos }D")
                        _input_pos = 0
                        sys.stdout.flush()
                elif seq in (b"F", b"4~"):
                    if _input_pos < n:
...
def _handle_input_char(ch: str):
    ...
    if ch in ("\x7f", "\x08"):
        with _print_lock:
            if _input_buffer and _input_pos > 0:
                del _input_buffer[_input_pos - 1]
                _input_pos -= 1
    ...
    if ch in ("\x13", "\x10"):  # Ctrl+S or Ctrl+P - scroll lock toggle (non-TUI)
        if _scroll_locked:
            ...
            with _print_lock:
                ...
                sys.stdout.write(
                    "\x1b[33m[PAUSED - PgUp/PgDn to scroll, Ctrl+P or Ctrl+S to resume]\x1b[0m\n"
                )
                sys.stdout.flush()
        return None
...
def _emit_line(text: str) -> None:
    ...
    try:
        with _print_lock:
            ...
            if len(_scroll_history) > 500:
                _scroll_history.pop(0)
            if _current_prompt or buf:
...
class OTRTracer:
    ...
    def trace(
        ...
        with self.lock:
            ...
            color = (
                "green"
                if "ESTABLISHED" in new_str or "ENCRYPTED" in new_str
                else (
                    "red"
                    if "FAILED" in new_str
                    else "yellow" if "SENT" in new_str or "RECEIVED" in new_str else "cyan"
                )
            )
...
class SecureMemory:
    ...
    def __init__(self, size: int):
        ...
        aligned_size = (
            (size + NetworkConstants.MLOCK_PAGE_SIZE - 1) // NetworkConstants.MLOCK_PAGE_SIZE
        ) * NetworkConstants.MLOCK_PAGE_SIZE
        with self._lock:
...
class ClientProfile:
    ...
    def __init__(self, identity_key=None, prekey=None):
        ...
        v10.6.12 (Phase 5.3e): identity keys are now Rust-owned by default.
        Both `self.identity_key` and `self.prekey` are
        `otrv4_core.Ed448KeyHandle` / `X448KeyHandle` objects whose private
        bytes live inside Rust SecretBytes (ZeroizeOnDrop) for the session
        lifetime.  Python receives only public bytes through the handle's
        `.public_bytes()` method.

        For backward compatibility (test code that constructs a ClientProfile
        with specific cryptography library keypairs), the legacy
        `Ed448PrivateKey` / `X448PrivateKey` parameters are still accepted.
        When provided, they are converted to Rust handles by extracting the
        raw bytes via the `from_seed_bytes` / `from_priv_bytes` static
        constructors.  This conversion path does touch the Python heap
        briefly (the bytearray we pass in is wiped after handle construction),
        but is only exercised by test code.
        ...
        if DEBUG_MODE:
            ...
            _sys.stderr.write(
                f"[ClientProfile] Fresh Rust-owned identity keys - "
                f"expires {time .ctime (self .expires )}\n"
            )
    ...
    def encode(self) -> bytes:
        try:
            ...
            if DEBUG_MODE:
                print(f"[ClientProfile] encode() → {len (result )} bytes ✅ (🦀 Rust handle)")
            return result
    ...
    def decode(cls, data: bytes) -> "ClientProfile":
        ...
        try:
            ...
            try:
                _sig_ok = _RustDAKE_module.verify_ed448_sig(
                    identity_pub_bytes, signed_data, signature
                )
            except ValueError as sig_err:

                raise ValueError(
                    f"ClientProfile signature verification FAILED: {sig_err }. "
                    "Rejecting profile - malformed identity key or signature "
                    "(OTRv4 §4.1.2)."
                ) from sig_err
            if not _sig_ok:
                raise ValueError(
                    "ClientProfile signature verification FAILED. "
                    "Rejecting profile - potential MITM/forged identity "
                    "(OTRv4 §4.1.2)."
                )
            ...
            if DEBUG_MODE:
                print(
                    f"[ClientProfile] ✅ Strict decode OK - sig verified, expires {time .ctime (expires )}"
                )
...
class TLVHandler:
    ...
    def debug_tlv(data: bytes, description: str = "") -> None:
        ...
        try:
            tlv_type, length = struct.unpack("!HH", data[:4])
            print(
                f"TLV {description }: type=0x{tlv_type :04x}, length={length }, data_len={len (data )}"
            )
...
class RustBackedDoubleRatchet:
    ...
    def from_dake_output(
        ...
        rekey_timeout: int = None,
    ) -> "RustBackedDoubleRatchet":
        ...
        if not RUST_RATCHET_AVAILABLE:
            raise RuntimeError(
                "otrv4_core Rust module not installed - Phase-4 ratchet " "requires the Rust core."
            )
    ...
    def encrypt_message(self, plaintext):
        ...
        with self.lock:
            now = time.time()
            if (
                self.message_counter_send >= self.rekey_interval
                or now - self.last_rekey_time > self.rekey_timeout
            ):
        ...
        with self.lock:
            ...
            try:
                ...
                if hdr_dh_pub is not None and hdr_msg_num is not None:
                    new_epoch = (
                        self._current_recv_dh_pub is None or hdr_dh_pub != self._current_recv_dh_pub
                    )
                    if new_epoch:
...
def _safe_b64decode(data: str) -> bytes:
    ...
    v10.7 (Phase 5.3h-D): rescued to module scope from the deleted
    pure-Python OTRv4DAKE class.  Every caller now uses the bare
    _safe_b64decode(...) function name.
    """
    try:
        data = str(data).strip()
        if not data:
        ...
        # OTRv4 DATA frames are terminated with a trailing '.' (the encode path
        # emits  "?OTRv4 <b64url>." ).  DAKE frames have no terminator.  Strip
        # any trailing dots here so every caller can pass the raw frame body
        # without having to know whether this particular message type is dotted.
        data = data.rstrip(".")

        # A url-safe base64 body stripped of '=' padding can only be 0, 2 or 3
        # (mod 4) characters long.  A length of 1 (mod 4) is impossible for any
        # valid encoding and is the fingerprint of a body that was decoded with
        # the STANDARD alphabet (which silently discards '-' and '_').  Fail
        # loudly instead of letting the b64decode fallback swallow real chars.
        if len(data) % 4 == 1:
            raise ValueError(
                f"corrupt base64: {len (data )} chars is 1 (mod 4); "
                "url-safe '-'/'_' were likely dropped by a standard-alphabet decode"
            )
        ...
        try:
            ...
            # Normalise url-safe alphabet to standard BEFORE stripping, so that
            # '-'/'_' become '+'/'/' rather than being discarded by b64decode.
            data = data.replace("-", "+").replace("_", "/")
            data = _re.sub(r"[^A-Za-z0-9+/=]", "", data)
            padding_needed = -len(data) % 4
            if padding_needed:
                data = data + "=" * padding_needed
            return base64.b64decode(data)
...
class RustDAKEAdapter:
    ...
    v10.7: the Rust otrv4_core.RustDAKE backend is mandatory.  The
    ...
    def __init__(
        ...
        if RUST_DAKE_AVAILABLE and _RustDAKE is not None:
            try:
                ...
                if (
                    self.client_profile.identity_pub_bytes is None
                    and self.client_profile.identity_key is not None
                ):
                    self.client_profile.identity_pub_bytes = bytes(
                        self.client_profile.identity_key.public_bytes()
                    )
                if (
                    self.client_profile.prekey_pub_bytes is None
                    and self.client_profile.prekey is not None
                ):
                    self.client_profile.prekey_pub_bytes = bytes(
                        self.client_profile.prekey.public_bytes()
                    )
                ...
                self._rust, _sig_py = _RustDAKE.sign_profile_body_and_construct_with_handles(
                    explicit_initiator,
                    _profile_bytes,
                    _unsigned_body,
                    self.client_profile.identity_key,
                    self.client_profile.prekey,
                    _mldsa_priv,
                    _mldsa_pub,
                )
                ...
                if len(_sig) != OTRConstants.ED448_SIGNATURE_SIZE:
                    raise ValueError(
                        f"sign_profile_body_and_construct_with_handles returned "
                        f"signature of wrong length: {len (_sig )} "
                        f"(expected {OTRConstants .ED448_SIGNATURE_SIZE })"
                    )

                if self.client_profile.signature != _sig:
                    raise ValueError(
                        "sign_profile_body_and_construct_with_handles produced "
                        "a different signature than ClientProfile.encode() - "
                        "Ed448 determinism violated, refusing to construct adapter"
                    )

                if logger:
                    logger.debug(
                        "RustDAKEAdapter: using Rust DAKE backend "
                        "(Phase 5.3e: Rust-owned identity key handles, "
                        "single-call sign+construct, no Python-heap private bytes)"
                    )
    ...
    def _fail(self, reason: str) -> bool:
        _prior = self._state.name if hasattr(self, "_state") else "NONE"
        self._state = DAKEState.FAILED
        self._trace("DAKE", "STATE", _prior, "FAILED", reason)
    ...
    def process_dake1(self, dake1_msg: str, peer_key: str = "unknown") -> bool:
        ...
        try:
            ...
            self._raw_dake1_bytes = raw
            self._log_debug(
                f"process_dake1: raw={len (raw )} bytes, first4={raw [:4 ].hex () if len (raw )>=4 else raw .hex ()}"
            )
            ...
            # ── Audit H3 (defense in depth): reject a DAKE1 whose initiator
            # ClientProfile does not verify.  This is already backstopped by
            # the remote_profile None-checks in generate_dake2 and
            # process_dake3, but failing here keeps the responder from doing
            # any further work on an unverified profile.
            if self.remote_profile is None:
                return self._fail(
                    "DAKE1: initiator ClientProfile verification failed "
                    "(bad signature, expiry, or downgrade)"
                )
        ...
        try:
            ...
            if self.remote_profile is None:
                return self._fail(
                    ...
                    "(bad signature, expiry, or downgrade) - refusing to establish"
                )
            # Bind the trusted identity to the VERIFIED profile, not the
            # unverified Rust extraction, and confirm the two agree.
            _verified_id = self.remote_profile.identity_pub_bytes
            if (
                self.remote_identity_pub_bytes is not None
                and not hmac.compare_digest(self.remote_identity_pub_bytes, _verified_id)
            ):
                return self._fail(
                    "DAKE2: extracted identity key does not match the signed profile"
                )
            self.remote_identity_pub_bytes = _verified_id
            self.remote_identity_key = _verified_id
    ...
    def process_dake3(self, dake3_msg: str) -> bool:
        ...
        try:
            ...
            self._session_created_at = time.time()
            self._trace(
                "DAKE", "STATE", "SENT_DAKE2", "ESTABLISHED", f"DAKE3 verified - {_pq_auth }"
            )
        ...
        with self._lock:
            try:
                ...
                if entry is not None:
                    stored_fp = entry.get("fingerprint", "")
                    if stored_fp and not hmac.compare_digest(
                        stored_fp.encode("utf-8"), fingerprint.encode("utf-8")
                    ):
                        raise TrustDatabase.FingerprintMismatchError(peer, stored_fp, fingerprint)
    ...
    def display(self, lines: List[str], header: str = "", footer: str = "",
        ...
        try:
            while self.active:
                ...
                safe_print(
                    colorize(f"── {header } ", "cyan")
                    + colorize(f"({page +1 }/{total_pages })" if total_pages > 1 else "", "dim")
                    + colorize(" " + "─" * 30, "cyan")
                )
            ...
            try:
                _cr = getattr(__import__("builtins"), "_active_client", None)
                if _cr and getattr(_cr, "_tui_enabled", False) and _cr._screen:
                    _cr._screen.redraw_full()
                elif _cr and _cr._prompt_refresh_cb:
...
class Screen:
    """Region-based terminal renderer for the OTRv4+ TUI (Phase 5.5)."""
    ...
    def _row_tab(self, tabs, i):
        return self.rows - 1 - (len(tabs) - 1 - i)
    ...
    def redraw_body(self) -> None:
        with _print_lock:
            ...
            out.append("\0338")
            out.append(f"\033[{_input_r };1H")
            _vis = len(_ANSI_RE.sub("", (_current_prompt or "") + _buf))
            if _vis > 0:
                out.append(f"\033[{_vis +1 }G")
    ...
    def redraw_tabbar(self) -> None:
        with _print_lock:
            ...
            for i, t in enumerate(tabs):
                out.append(f"\033[{self ._row_tab (tabs ,i )};1H\033[K")
                out.append(t)
            out.append("\0338")
            sys.stdout.write("".join(out))
            sys.stdout.flush()
...
class PanelManager:
    ...
    def update_panel_security(self, name: str, level: UIConstants.SecurityLevel) -> None:
        ...
        with self.lock:
            ...
            if level in (
                UIConstants.SecurityLevel.ENCRYPTED,
...
class MessageRouter:
    ...
    def route(
        self, message: str, prefix: Optional[str] = None, msg_type: Optional[str] = None
    ) -> str:
...
class EnhancedOTRSession:
    ...
    def __init__(
        self, peer: str, is_initiator: bool, tracer: OTRTracer, logger: Optional[OTRLogger] = None
    ):
        self.peer = peer
        self.is_initiator = is_initiator
        self.tracer = tracer
    ...
    def transition_smp(self, new_state: UIConstants.SMPState, reason: str = ""):
        ...
        try:
            ...
            if new_state == UIConstants.SMPState.SUCCEEDED:
                ...
                self.tracer.trace(
                    self.peer, "SECURITY", "ENCRYPTED", "SMP_VERIFIED", "SMP succeeded"
                )
    ...
    def initialize_dake(
        self, client_profile: ClientProfile, explicit_initiator: bool = False
    ) -> "RustDAKEAdapter":
    ...
    def _initialize_ratchet(self):
        ...
        try:
            ...
            if dake_output is not None and not getattr(dake_output, "consumed", True):
                ...
                self.tracer.trace(
                    self.peer, "RATCHET", None, "ACTIVE", f"ratchet: {_backend_label }"
                )
                return
    ...
    def queue_outgoing_message(self, message: str):
        ...
        try:
            if self.session_state == SessionState.ENCRYPTED:
                ...
                raise StateMachineError(
                    f"Cannot send message in state: {self .session_state .name }"
                )
        finally:
            self._release_lock()
    ...
    def _process_queued_messages(self):
        ...
        try:
            ...
            for msg in self.pending_messages:
                self.tracer.trace(
                    self.peer, "QUEUE", "QUEUED", "PROCESSED", f"message: {msg [:50 ]}..."
                )
    ...
    def decrypt_message(self, encrypted_msg: str) -> bytes:
        ...
        try:
            ...
            # Single decoder of record: _safe_b64decode strips the trailing '.'
            # terminator, decodes url-safe, and rejects 1-mod-4 corruption.
        ...
        for tlv in tlvs:
            try:
                if tlv.type == OTRv4TLV.DISCONNECTED:
                    ...
                    self.tracer.trace(
                        self.peer, "DISCONNECT", "TLV", "RECEIVED", "peer ended session gracefully"
                    )
                    if self.session_state == SessionState.ENCRYPTED:
                        try:
                            self.transition_session(
                                SessionState.FINISHED, "peer sent DISCONNECTED TLV"
                            )
    ...
    def _enh_handle_smp_tlv(self, tlv: "OTRv4TLV") -> None:
        ...
        try:
            if tlv.type in (OTRv4TLV.SMP_MSG_1, OTRv4TLV.SMP_MSG_1Q):
                ...
                self.smp_step = 2
                self._smp_progress_notify(
                    2, 4, "Response received - verifying ML-DSA-87 + ZKP…", role="initiator"
                )
                ...
                self._smp_progress_notify(
                    3,
                    4,
                    "PQ proof verified - sending confirmation (ML-DSA-87 + ZKP)…",
                    role="initiator",
                )
                ...
                if self.rust_smp.is_verified():
                    ...
                    self.security_level = UIConstants.SecurityLevel.SMP_VERIFIED
                    self.tracer.trace(
                        self.peer, "SMP", "VERIFIED", "STATE_UPDATED", "role=responder"
                    )
                    self._smp_progress_notify(
                        4,
                        4,
                ...
                if verified:
                    ...
                    self.smp_step = 0
                    self.tracer.trace(
                        self.peer, "SMP", "FAILED", "STATE_UPDATED", "secrets did not match"
                    )
                    self._smp_progress_notify(
                        0,
                        4,
    ...
    def start_dake(self) -> Optional[str]:
        ...
        try:
            ...
            self.tracer.trace(
                self.peer, "OPPORTUNISTIC", "IDLE", "STARTED", "first outgoing message"
            )
    ...
    def start_smp(self, secret: str, question: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            self.smp_step = 1
            self._smp_progress_notify(
                1, 4, "Challenge sent - awaiting response (Rust ZKP)…", color="yellow"
            )
        ...
        try:
            ...
            if tlv_type in (OTRv4TLV.SMP_MSG_1, OTRv4TLV.SMP_MSG_1Q):
                ...
                if verified:
                    ...
                    self.security_level = UIConstants.SecurityLevel.SMP_VERIFIED
                    self.tracer.trace(
                        self.peer, "SMP", "VERIFIED", "STATE_UPDATED", "role=initiator"
                    )
                    self._smp_progress_notify(
                        4,
                        4,
    ...
    def get_smp_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "has_question": False,
                "question": "",
                "can_start_smp": phase in ("IDLE", "FAILED")
                and self.session_state == SessionState.ENCRYPTED,
                "can_retry": failed,
                "retry_count": 0,
                "expired": False,
    ...
    def should_start_dake(self) -> bool:
        ...
        return (
            self.session_state == SessionState.PLAINTEXT
            and self.dake_state == DAKEState.IDLE
            and self.is_initiator
        )
...
class SessionManager:
    ...
    def start_session(self, peer: str) -> Optional[str]:
        ...
        try:
            ...
            dake = RustDAKEAdapter(
                client_profile=self.client_profile, explicit_initiator=True, logger=self.logger
            )
    ...
    def handle_dake1(self, peer: str, dake1_msg: str) -> Optional[str]:
        ...
        try:
            dake = RustDAKEAdapter(
                client_profile=self.client_profile, explicit_initiator=False, logger=self.logger
            )
    ...
    def get_smp_status(self, peer: str) -> Dict[str, Any]:
        ...
        if hasattr(session, "get_smp_status"):
            ...
            status["can_start_smp"] = status.get(
                "can_start_smp",
...
class EnhancedSessionManager:
    ...
    def __init__(
        self,
        config: Optional[OTRConfig] = None,
        tracer: Optional[OTRTracer] = None,
        logger: Optional[OTRLogger] = None,
    ):
        self.config = config or OTRConfig(test_mode=True)
        ...
        with self.lock:
            ...
            if session.is_encrypted():
                ...
                if session.should_start_dake():
                    self.tracer.trace(
                        peer, "OPPORTUNISTIC", "IDLE", "STARTING", f"first message to {peer }"
                    )

                    dake_engine = session.initialize_dake(
                        self.client_profile, explicit_initiator=True
                    )
                    ...
                    self.tracer.trace(
                        peer, "ERROR", "SEND", "BLOCKED", f"plaintext session but DAKE not started"
                    )
                    return None, False
                ...
                self.tracer.trace(
                    peer, "QUEUE", "MESSAGE", "QUEUED", f"DAKE in progress, message queued"
                )
    ...
    def _handle_otr_message(self, peer: str, message: str) -> Optional[bytes]:
        ...
        with self.lock:
            try:
                # NB: route through _safe_b64decode rather than a bespoke
                # decoder here.  DATA frames carry a trailing '.' terminator and
                # the body uses the URL-SAFE alphabet ('-'/'_'); _safe_b64decode
                # strips the dot and decodes url-safe correctly.  The previous
                # inline decoder left the '.' in place, so urlsafe_b64decode hit
                # "Incorrect padding" and the standard-b64decode fallback then
                # silently dropped every '-'/'_' (and the dot), yielding a
                # 1-mod-4 length and the "cannot be 1 more than a multiple of 4"
                # crash on the very first plaintext DATA message.
                ...
                if msg_type == OTRConstants.MESSAGE_TYPE_DAKE1:
                    ...
                    self.tracer.trace(
                        peer, "ERROR", "PARSE", "UNKNOWN", f"unknown message type: {msg_type }"
                    )
                    return None
        ...
        with self.lock:
            ...
            try:
                ...
                if isinstance(pub_key_data, bytes):
                    ...
                    self.tracer.trace(
                        peer, "KEY", "PUBKEY", "PARSED", "Successfully parsed remote pubkey (bytes)"
                    )
        ...
        if session.session_state == SessionState.PLAINTEXT:
            try:
                session.transition_session(
                    SessionState.DAKE_IN_PROGRESS, f"{reason }: fast-path to DAKE_IN_PROGRESS"
                )
            except StateMachineError:
                pass
    ...
    def _handle_data_message(self, peer: str, data_msg: str) -> Optional[bytes]:
        ...
        with self.lock:
            ...
            try:
                ...
                self.tracer.trace(
                    peer, "RECEIVE", "ENCRYPTED", "DECRYPTED", f"len={len (text_bytes )}"
                )
    ...
    def trust_fingerprint(self, peer: str, fingerprint: str) -> bool:
        ...
        with self.lock:
            ...
            if success:
                self.tracer.trace(
                    peer, "TRUST", "UNTRUSTED", "TRUSTED", f"fingerprint: {fingerprint [:16 ]}..."
                )
            else:
                self.tracer.trace(
                    peer, "ERROR", "TRUST", "FAILED", "could not add to trust database"
                )
        ...
        with self.lock:
            ...
            try:
                ...
                needs_bind = smp_phase in ("IDLE", "FAILED") and (
                    sess.rust_smp is None or not sess.rust_smp.check_secret_set()
                )
                if needs_bind:
...
class ErrorEvent(Event):
    ...
    def __init__(self, error_type: str, peer: str, error: Exception, context: Dict[str, Any]):
        super().__init__(
            "ERROR",
            {"error_type": error_type, "peer": peer, "error": str(error)[:100], "context": context},
        )
...
class SMPEvent(Event):
    ...
    def __init__(self, smp_type: str, peer: str, session_id: str, details: Dict[str, Any]):
        super().__init__(
            "SMP",
            {"smp_type": smp_type, "peer": peer, "session_id": session_id, "details": details},
        )
...
class EventHandler:
    ...
    def emit_security_event(
        self, security_type: str, peer: str, session_id: str, details: Dict[str, Any]
    ):
...
class OTRFragmentBuffer:
    ...
    _LEGACY_RE = re.compile(r"^\?OTRv4\s*" r"(?:\[(\d+)/(\d+)\])?" r"(.*?)(?:\.?)$", re.DOTALL)
    ...
    def _buffer(self, sender: str, idx: int, total: int, chunk: str, now: float) -> Optional[str]:
        ...
        if idx not in state["parts"]:
            ...
            _MAX_REASSEMBLED_BYTES = 1_048_576
            ...
            if total > 1:
                try:
                    got = len(state["parts"])
                    _rb = "█" * got + "░" * (total - got)
                    _set_prompt(f"\x1b[36m⟳ RX {sender } [{got }/{total }] {_rb }\x1b[0m ")
                except Exception:
                    pass
        ...
        if len(state["parts"]) == total:
            ...
            try:
                _cr = getattr(__import__("builtins"), "_active_client", None)
                if _cr and _cr._prompt_refresh_cb:
                    _cr._prompt_refresh_cb()
            except Exception:
                pass
    ...
    def get_pending_for(self, sender: str) -> int:
        ...
        with self._lock:
            return sum(
                len(state["parts"]) for (s, _t), state in self._buffers.items() if s == sender
            )
    ...
    def _expire(self, now: float) -> None:
        if len(self._buffers) > self.max_total_senders:
            ...
            for key, _ in oldest[: len(self._buffers) - self.max_total_senders]:
                if DEBUG_MODE:
                    _emit_line(
                        f"[fragment] buffer evict: too many senders, dropping {key [0 ]}/{key [1 ]}"
                    )
                del self._buffers[key]
        cutoff = now - self.timeout
        expired = [
            k
            for k, st in self._buffers.items()
            if st.get("last_ts", st.get("first_ts", 0)) < cutoff
        ]
        for k in expired:
            ...
            if DEBUG_MODE:
                _emit_line(
                    f"[fragment] timeout: {sender_nick } {got }/{frag_total } frags - type /otr {sender_nick } to retry"
                )
...
class OTRMessageFragmenter:
    ...
    def fragment(
        ...
        if DEBUG_MODE:
            _c = getattr(__import__("builtins"), "_active_client", None)
            _msg = f"[OTRMessageFragmenter] {len (fragments )} frags from {len (payload )} chars"
            if _c:
                _c._emit("debug", _msg)
            else:
                _emit_line(_msg)
    ...
    @staticmethod
    def fragment_otr_message(
        otr_message: str, fragment_size: int = UIConstants.OTR_FRAGMENT_SIZE
    ) -> List[str]:
...
class OTRv4IRCClient:
    ...
    def _emit(self, panel: str, message: str) -> None:
        ...
        if panel not in self.panel_manager.panels:
            ptype = (
                "channel"
                if panel.startswith("#")
                else "debug" if panel == "debug" else "system" if panel == "system" else "private"
            )
        ...
        _is_background_tab = (
            ...
            and panel != self.panel_manager.active_panel
            and panel != "system"
            and panel != "debug"
        )
        ...
        if panel == "system":
            ...
            _sec = (
                self.session_manager.get_security_level(panel)
        ...
        *net_type* is one of NetworkConstants.NET_* constants.
        ...
        if net_type == NetworkConstants.NET_I2P:
            ...
            self.add_message(
                "system",
                colorize(
                    f"🧅 I2P SOCKS5 proxy: {_sanitise (str (host ),64 )}:{port }", "dark_cyan"
                ),
            )
            ...
            socket.socket = socks.socksocket
            self.add_message(
                "system", colorize(f"🧅 Tor SOCKS5 proxy: {host }:{port }", "dark_magenta")
            )
            ...
            if net_type == NetworkConstants.NET_CLEARNET:
                ...
                if port == IRCConstants.PORT and not self.config.use_tls:
                    ...
                    self.add_message(
                        "system",
                        colorize(
                            "⚠ Plaintext port 6667 - most networks now "
        ...
        while self.running and not self.shutdown_flag:
            try:
                ...
                if not chunk:
                    if self.running:
                        self.add_message(
                            "system",
                            colorize(
                                "⚠ Server closed the connection. " "Reconnecting automatically…",
                                "yellow",
                            ),
                        )
                        self._try_reconnect()
                ...
                if now - self.last_ping > 600:
                    self.add_message(
                        "system", colorize("⚠ Ping timeout. Reconnecting automatically…", "yellow")
                    )
                    self._try_reconnect()
                    return
                if hasattr(self, "_msg_rate") and len(self._msg_rate) > 500:
                    self._msg_rate.clear()
                _has_otr = hasattr(self, "session_manager") and bool(
                    self.session_manager.list_encrypted_sessions()
                )
                ...
                try:
                    ...
                    if hasattr(self, "session_manager") and hasattr(self, "_last_otr_sent"):
                        for peer, sess in list(self.session_manager.sessions.items()):
                            if (
                                getattr(sess, "session_state", None) is not None
                                and sess.session_state.name == "ENCRYPTED"
                            ):
                ...
                if self.running:
                    ...
                    if _uptime < 3.0 and _is_clearnet_plain and "104" in str(exc):
                        self.add_message(
                            "system",
                            colorize(
                                "⚠ Server reset plaintext connection - "
                                "auto-upgrading to TLS on port 6697…",
                                "yellow",
                            ),
                        )
                        ...
                        self.add_message(
                            "system",
                            colorize(
                                f"⚠ Connection lost ({exc }). " "Reconnecting automatically…",
                                "yellow",
                            ),
                        )
                    self._try_reconnect()
        ...
        for i, frag in enumerate(fragments):
            ...
            if len(fragments) > 1:
                try:
                    ...
                    _set_prompt(
                        f"\x1b[36m↑ TX {target } [{i +1 }/{len (fragments )}] {_tb }\x1b[0m "
                    )
                except Exception:
                    pass
            if not self.send(f"PRIVMSG {target } :{frag }"):
                ...
                if len(fragments) > 1:
                    self.add_message(
                        target if target in self.panel_manager.panels else "system",
                        colorize(
                            f"⚠ Fragment send failed at {i +1 }/{len (fragments )} - /otr {target } to retry",
                            "yellow",
                        ),
                    )
                break
    ...
    def handle_message(self, line: str):
        ...
        try:
            ...
            self.debug(
                "recv", {"cmd": command, "params": params[:3], "trail": (trailing or "")[:120]}
            )
            ...
            if command == "PRIVMSG":
                ...
                if "?OTRv4" in message:
                    ...
                    if _count > 50:
                        self.debug(
                            f"plaintext flood from {sender } - dropping ({_count } msgs/10s)"
                        )
                    else:
            ...
            if command == "JOIN":
                ...
                if sender == self.nick:
                    ...
                    self.add_message(
                        channel, colorize(f"✅ Joined {_sanitise (channel ,64 )}", "green")
                    )
                else:
            ...
            if command == "PART":
                ...
                if sender == self.nick:
                    ...
                    if channel in self.panel_manager.panels:
                        _was = self.panel_manager.active_panel == channel
                        ...
                        elif getattr(self, "_tui_enabled", False) and self._screen:
            ...
            if command == "KICK":
                ...
                if kicked == self.nick:
                    self.add_message(
                        "system",
                        f"❌ Kicked from {_sanitise (channel ,64 )}: {_sanitise (reason ,256 )}",
                    )
                    self.channels.pop(channel, None)
                    if channel in self.panel_manager.panels:
                    ...
                    self.add_message(
                        channel,
                        f"⚡ {colorize_username (_sanitise (kicked ,64 ))} kicked: {_sanitise (reason ,256 )}",
                    )
                return
            ...
            if command == "NOTICE":
                ...
                if sender_lower == "nickserv":
                    self.add_message(
                        "system",
                        colorize("NickServ", "bold_cyan")
                        + colorize(": ", "dim")
                        + colorize(_sanitise(message, 512), "white"),
                    )
                    msg_lower = message.lower()
                    if (
                        ("you are now" in msg_lower and "identified" in msg_lower)
                        or "password accepted" in msg_lower
                        or "you are successfully identified" in msg_lower
                    ):
                        self.nickserv_identified = True
                        self.add_message(
                            "system", colorize("✅ NickServ: identified successfully", "green")
                        )
                    elif "registered" in msg_lower and (
                        "successfully" in msg_lower or "is now" in msg_lower
                    ):
                        self.nickserv_identified = True
                        self.add_message(
                            "system", colorize("✅ NickServ: nick registered successfully", "green")
                        )
                    elif (
                        "invalid" in msg_lower
                        or "denied" in msg_lower
                        or "incorrect" in msg_lower
                        or "wrong" in msg_lower
                    ):
                        self.add_message(
                            "system", colorize("❌ NickServ: authentication failed", "red")
                        )
                elif sender_lower in ("chanserv", "memoserv", "operserv"):
                    self.add_message(
                        "system",
                        colorize(_sanitise(sender, 64), "dim")
                        + colorize(": ", "dim")
                    ...
                    if message and not message.startswith("***"):
                        self.add_message(
                            "system",
                            colorize(
                                f"[notice] {_sanitise (sender ,64 )}: {_sanitise (message ,512 )}",
                                "dim",
                            ),
                        )
                return
    ...
    def handle_numeric_reply(self, code: int, params: List[str], trailing: Optional[str]):
        ...
        try:
            ...
            if code in (902, 904, 905, 906):
                ...
                self.add_message(
                    "system", colorize(f"⚠ SASL authentication failed (code {code })", "yellow")
                )
                if getattr(self, "_sasl_in_progress", False):
                    self._sasl_in_progress = False
                    self._finish_cap_negotiation()
            ...
            if code == 1:
                ...
                if self.config.nickserv_login and self.config.nickserv_pass:
                    ...
                    self.add_message(
                        "system", colorize("📝 Registering nick with NickServ…", "cyan")
                    )
                    _ns_pass = self.config.nickserv_pass
                ...
                delay = (
                    3.0 if (self.config.nickserv_login or self.config.nickserv_register) else 2.0
                )
                    ...
                    for para in paragraphs:
                        if len(para) > 68:
                            for wline in textwrap.wrap(
                                para, width=68, break_long_words=False, break_on_hyphens=False
                            ):
                ...
                if getattr(self, "_who_pending", False):
                    ...
                    if _wc < _WHO_DISPLAY_MAX:
                        ...
                        self.add_message(
                            _wp2,
                            colorize(
                                f"  … output capped at {_WHO_DISPLAY_MAX } rows - use /who in a smaller channel",
                                "yellow",
                            ),
                        )
                        self._who_count += 1
            ...
            if code == 303:
                ...
                if _online.strip():
                    self.add_message(
                        _wp5, colorize(f"ISON online: {_sanitise (_online ,256 )}", "green")
                    )
                else:
            ...
            if code == 314:
                ...
                self.add_message(
                    _wp4, colorize("── WHOWAS ───────────────────────────────────", "dim")
                )
                self.add_message(
                    _wp4,
                    f"  {colorize_username (_ww_nick )}  {_sanitise (_ww_user +'@'+_ww_host ,80 )}  {_sanitise (_ww_real ,60 )}",
                )
                return
            if code == 315:
                if getattr(self, "_who_pending", False):
            ...
            if code == 312:
                ...
                info = trailing or ""
                self.add_message(
                    getattr(self, "_whois_panel", "system"),
                    f"  Server   : {_sanitise (server ,128 )}"
                    + (f" ({_sanitise (info ,256 )})" if info else ""),
                )
                return
            ...
            if code == 317:
                ...
                if signon > 0:
                    ...
                    self.add_message(
                        getattr(self, "_whois_panel", "system"), f"  Idle     : {idle_str }"
                    )
                    self.add_message(
                        getattr(self, "_whois_panel", "system"), f"  Signon   : {signon_str }"
                    )
                else:
                    self.add_message(
                        getattr(self, "_whois_panel", "system"), f"  Idle     : {idle_str }"
            ...
            def _on_first_fragment(s: str, n: int, chunk: str = "") -> None:
                ...
                try:
                    ...
                    try:
                        ...
                        # Peek only needs the first few decoded bytes to read the
                        # message-type tag.  Use the URL-SAFE alphabet (OTRv4
                        # bodies contain '-'/'_'); a standard b64decode would
                        # drop those chars and could mis-tag the fragment.  Slice
                        # on a 4-char boundary so the truncated body stays valid
                        # base64, then pad.
                        _peek_src = chunk[:16]
                    ...
                    _should_switch = msg_type in (
                        "dake1",
                        "dake2",
                        "dake3",
                    ) or not self.session_manager.has_session(s)
                    if _should_switch:
                    ...
                    if msg_type == "dake1" or not self.session_manager.has_session(s):
                        notice = colorize(
                            f"🔑 {colorize_username (s )} is requesting an OTR session…", "cyan"
                        )
                        ...
                        self._termux_fire(
                            [
                                ...
                                "🔑 OTR request",
                                "--content",
                                f"{s } is requesting an encrypted session",
                                "--priority",
                                "high",
                                "--id",
                                f"otrv4_{s }_incoming",
                                "--vibrate",
                                "0,150,100,150",
                            ]
                        )
    ...
    def _handle_data_message(self, sender: str, payload: str):
        ...
        def _do_decrypt():
            ...
            try:
                ...
                # SECURITY: a decrypted message is peer-controlled content. It is
                # passed through _sanitise() before it reaches the terminal, just
                # like the plaintext path, so a hostile peer cannot hijack the
                # terminal title, reposition the cursor, or inject control codes
                # through an otherwise "trusted" encrypted message.
                _safe_text = _sanitise(text, 512)
                if _safe_text.strip():
                    self.add_message(sender, f"{colorize_username(sender)}: {_safe_text}", sec)
    ...
    def _display_plaintext(self, sender: str, target: str, message: str):
        ...
        if message.startswith("\x01ACTION ") and message.endswith("\x01"):
            ...
            self.add_message(
                panel, colorize(f"* {sender } {_sanitise (message [8 :-1 ],490 )}", "cyan")
            )
            return
        panel = target if target.startswith("#") else sender
        ...
        if not target.startswith("#"):
            ...
            if _active and _active != panel and _active not in ("system", "debug"):
                _hint = colorize(
                    f"\U0001f4ac PM from {sender } - /switch {sender } to view", "yellow"
                )
    ...
    def process_dake1(self, sender: str, payload: str):
        ...
        alert = colorize(
            f"🔑 OTR request from {_sanitise (sender ,64 )} - switching to their tab…", "cyan"
        )
        if active:
    ...
    def start_auto_smp_monitor(self):
        ...
        def _monitor():
            while self.running and not self.shutdown_flag:
                ...
                due = [
                    peer
                    for peer, info in list(self.smp_schedule_timers.items())
    ...
    def _switch_panel(self, name: str) -> bool:
        ...
        if _switched_panel:
            ...
            if len(_scroll_history) > 500:
                _scroll_history = _scroll_history[-500:]
        if getattr(self, "_tui_enabled", False) and self._screen is not None:
    ...
    def show_status_panel(self):
        ...
        self.add_message(
            "system",
            f"  IRC ready : {colorize ('YES','green')if self .auth_complete else colorize ('NO','yellow')}{ns_status }",
        )
    ...
    def handle_chat_message(self, msg: str):
        ...
        if self.session_manager.has_session(target):
            ...
            if enc and self.send_otr_message(target, enc):
                ...
                self.add_message(
                    target,
                    colorize_username(self.nick) + colorize(": ", "dim") + colorize(msg, "white"),
                    sec,
                )
            else:
    ...
    def handle_command(self, command: str):
        ...
        if cmd == "help":
            ...
            _new_nick = (
                _sanitise(parts[1], 64).split()[0] if _sanitise(parts[1], 64).split() else ""
            )
            if _new_nick:
            ...
            if _msg_target:
                ...
                self.add_message(
                    _msg_target,
                    colorize_username(self.nick)
                    + colorize(": ", "dim")
                    + colorize(_msg_text, "white"),
                )
        elif cmd == "list":
            self.send("LIST")
            ...
            _inv_chan = (
                _sanitise(_inv_chan, 64).split()[0] if _sanitise(_inv_chan, 64).split() else ""
            )
            ...
            if len(parts) > 1:
                ...
                _mode_args = [
                    _sanitise(p, 64).split()[0] for p in parts[1:] if _sanitise(p, 64).split()
                ]
                if _mode_args:
            ...
            _t = parts[1] if len(parts) > 1 else (self.panel_manager.active_panel or "")
            ...
            self._who_count = 0
            self.add_message(
                self.panel_manager.active_panel or "system",
                colorize("── WHO (max 200 rows) ────────────────────────────────────", "dim"),
            )
        elif cmd == "whowas" and len(parts) > 1:
            _w = _sanitise(parts[1], 64).split()[0]
            if _w:
                self.send(f"WHOWAS {_w }")
        elif cmd == "ison" and len(parts) > 1:
            self.send(f"ISON {' '.join (_sanitise (p2 ,64 )for p2 in parts [1 :])}")
        elif cmd == "userhost" and len(parts) > 1:
            self.send(f"USERHOST {' '.join (_sanitise (p2 ,64 )for p2 in parts [1 :6 ])}")
        elif cmd == "motd":
            self.send("MOTD")
        elif cmd == "time":
            self.send("TIME")
        elif cmd == "me" and len(parts) > 1:
            _t2 = self.panel_manager.active_panel or ""
            if _t2 and _t2 not in ("system", "debug"):
                _mt = _sanitise(" ".join(parts[1:]), 490)
                self.send(f"PRIVMSG {_t2 } :\x01ACTION {_mt }\x01")
                self.add_message(_t2, colorize(f"* {self .nick } {_mt }", "cyan"))
        elif cmd == "ctcp" and len(parts) > 2:
            _cn = _sanitise(parts[1], 64).split()[0]
            _cc = _sanitise(parts[2], 32).upper()
            _ca = (" " + _sanitise(" ".join(parts[3:]), 400)) if len(parts) > 3 else ""
            if _cn:
                self.send(f"PRIVMSG {_cn } :\x01{_cc }{_ca }\x01")
        elif cmd == "cycle":
            _cy = parts[1] if len(parts) > 1 else self.panel_manager.active_panel or ""
            if _cy and not _cy.startswith("#"):
                _cy = "#" + _cy
            if _cy:
                self.send(f"PART {_cy } :cycling")
                import time as _t3

                _t3.sleep(0.5)
                self.send(f"JOIN {_cy }")
        elif cmd == "op" and len(parts) > 1:
            _ch2 = self.panel_manager.active_panel or ""
            if _ch2.startswith("#"):
                self.send(f"MODE {_ch2 } +{'o'*len (parts [1 :])} {' '.join (parts [1 :])}")
            ...
            _ch2 = self.panel_manager.active_panel or ""
            if _ch2.startswith("#"):
                self.send(f"MODE {_ch2 } +{'v'*len (parts [1 :])} {' '.join (parts [1 :])}")
            ...
            _ch2 = self.panel_manager.active_panel or ""
            if _ch2.startswith("#"):
                self.send(f"MODE {_ch2 } +b {_sanitise (parts [1 ],128 )}")
            ...
            _ch2 = self.panel_manager.active_panel or ""
            _kbn = _sanitise(parts[1], 64).split()[0]
            _kbr = _sanitise(" ".join(parts[2:]), 200) if len(parts) > 2 else "banned"
            if _ch2.startswith("#") and _kbn:
                self.send(f"MODE {_ch2 } +b {_kbn }!*@*")
                self.send(f"KICK {_ch2 } {_kbn } :{_kbr }")
        elif cmd == "accept" and len(parts) > 1:
            self.send(f"ACCEPT {_sanitise (parts [1 ],64 )}")
            ...
            if _mop == "l":
                ...
                self.send(
                    f"MONITOR {_mop .upper ()} {','.join (_sanitise (p2 ,64 )for p2 in parts [2 :])}"
                )
        elif cmd == "setname" and len(parts) > 1:
            self.send(f"SETNAME :{_sanitise (' '.join (parts [1 :]),128 )}")
        elif cmd == "server" and len(parts) > 1:
            _sv = _sanitise(parts[1], 256).split()[0]
            try:
                _sp = int(parts[2]) if len(parts) > 2 else self.config.port
            except ValueError:
                _sp = self.config.port
            if _sv:
                self.config.server = _sv
                self.config.port = _sp
                self.server = _sv
                self.add_message("system", colorize(f"Switching to {_sv }:{_sp }…", "cyan"))
                self._try_reconnect()
            ...
            fp = (
                self.session_manager.client_profile.get_fingerprint()
            ...
            self.add_message(
                "system", "SMP    : 🦀 Rust (ML-KEM-1024 + ML-DSA-87 hybrid PQC, ZeroizeOnDrop)"
            )
        elif cmd == "pause":
            global _scroll_locked
            _scroll_locked = True
            ...
            if cmd == "/server":
                if not parts[1:]:
                    ...
                    if not self.connect():
                        self.add_message(
                            "system", colorize(f"❌ Failed to connect to {new_srv }", "bold_red")
                        )
                return

            self.add_message(
                "system", colorize(f"❌ Unknown command: {cmd }  (try /help)", "bold_red")
            )
    ...
    def show_help(self, topic=None):
        ...
        if topic in ("mode", "modes"):
            ...
            lines = [
                ...
                "    +g  Caller-ID - must /accept nick before they can PM you",
                "    +R  Block PMs from unregistered users",
                "    +w  Receive wallops (server announcements)",
                "    +x  Cloak your hostname (hide IP in /whois)",
                "",
            ...
            header = "IRC Commands"
            lines = [
                "  /join <#ch>          Join channel(s) - /join a,b,c",
                ...
                "  /whowas <nick>       Recent disconnected nick",
                "  /ison <nicks>        Check who is online",
                "  /topic [#ch] [text]  View or set topic",
                "  /invite <n> <#ch>    Invite to channel",
                "  /kick <nick> [why]   Kick from channel",
                "  /mode <t> <flag>     Set mode (/help mode)",
                "  /notice <t> <msg>    Send notice",
                "  /away [msg]          Set away",
                "  /back                Clear away",
                "  /me <text>           CTCP ACTION (/me waves)",
                "  /ctcp <n> <cmd>      Send CTCP request",
                "  /cycle [#ch]         Part then rejoin",
                "  /op <nick>           Give op status",
                "  /deop <nick>         Remove op status",
                "  /voice <nick>        Give voice",
                "  /devoice <nick>      Remove voice",
                "  /ban <mask>          Set ban",
                "  /unban <mask>        Remove ban",
                "  /kickban <nick>      Ban + kick",
                "  /accept <nick>       Add to caller-ID list (+g)",
                "  /monitor + <nick>    Watch for nick online/offline",
                "  /setname <text>      Change your realname",
                "  /server <host>       Switch server",
                "  /raw <cmd>           Raw IRC command",
                "  /motd                Fetch server MOTD",
                "  /time                Server time",
                "  /reconnect           Reconnect",
            ...
            lines = [
                ...
                "  /tui on|off          Force TUI on or off",
            ...
            lines = [
                ...
                "    /quit              Exit",
                "",
                colorize("  Help topics (type /help <topic>):", "yellow"),
                "    /help irc    - all IRC commands",
                "    /help otr    - OTR encryption",
                "    /help mode   - IRC modes (+g, +i, etc)",
                "    /help ui     - TUI and navigation",
                "",
    ...
    def start_guided_otr_session(self, nick: str):
        ...
        self.add_message(
            nick, colorize(f"🔑 Initiating OTR with {colorize_username (nick )}…", "cyan")
        )
        ...
        if dake_msg and should_send:
            ...
            self.add_message(
                nick, colorize(f"🔑 DAKE1 → sent - waiting for response…", "bold_cyan")
            )
...
class EnhancedOTRv4IRCClient(OTRv4IRCClient):
    ...
    def process_dake1(self, sender: str, payload: str):
        """DAKE1 reassembled - process as responder.

        _on_first_fragment already created the peer tab and switched to it
        on fragment 1.  By the time we arrive here all fragments are in.
        """
    ...
    def _handle_session_established(self, peer: str, is_initiator: bool):
        ...
        try:
            ...
            _session = (
                self.session_manager.get_session(peer)
            ...
            _dake_tag = "🦀 Rust"
            _smp_tag = (
                "🦀 Rust" if (getattr(_session, "rust_smp", None) is not None) else "🐍 Python"
            )
            ...
            self.add_message(
                peer, f"  Theirs: {colorize (self ._fmt_fp (remote_fp ),'yellow')}", sec
            )
            self.add_message(peer, colorize("─" * 50, "dim"), sec)
    ...
    def _consume_secret_line(self, text: str) -> bool:
        ...
        try:
            ...
            self.add_message(peer, colorize(
                ...
                % type(exc).__name__, "red"), self._panel_sec(peer))
            try:
                flow.reset()
            except _smpflow.SmpFlowError:
                pass
    ...
    def _resume_smp(self, peer: str) -> None:
        ...
        if not smp2:
            ...
            return
        try:
            self._smp_flows.get(peer).running()
        except _smpflow.SmpFlowError:
            pass
    ...
    def _finish_trust(self, peer: str, trusted: bool, remote_fp: str, is_initiator: bool):
        ...
        if trusted:
            ...
            sec = UIConstants.SecurityLevel.ENCRYPTED
            self.add_message(
                peer, colorize("🟡 Fingerprint NOT trusted - encrypted only", "yellow"), sec
            )
    ...
    # Known termux-notification option flags. Any argument NOT in this set is
    # treated as untrusted data (a title/body/id derived from a nick or channel)
    # and is sanitised before it can reach the subprocess.
    ...
    @staticmethod
    def _termux_clean(value: str, maxlen: int = 256) -> str:
        """Neutralise untrusted text before it becomes a subprocess argument.

        Strips ANSI/CSI escape sequences and C0/C1 control characters (including
        NUL and newlines) so a hostile nick, channel, or message body cannot
        spoof a notification or smuggle terminal control codes, and caps the
        length so a single notification cannot be abused. Printable Unicode
        (including emoji) is preserved.
        """
        text = str(value)
        text = re.sub(r"\x1b\[[0-9;?]*[ -/]*[@-~]", "", text)
        text = "".join(
            ch for ch in text if ch == "\t" or (0x20 <= ord(ch) < 0x7F) or ord(ch) >= 0xA0
        )
        return text[:maxlen]
    ...
    def _termux_fire(self, args: list) -> None:
        ...
        Every argument is hardened against option-injection: fixed flags pass
        through unchanged, while any value is sanitised and a non-flag value that
        begins with '-' is de-fanged, so attacker-controlled text can never be
        parsed by termux-notification as a command-line option.
        """
        try:
            ...
            safe: list = []
            for a in args:
                a = self._termux_clean(a)
                if a not in self._TERMUX_FLAGS and a.startswith("-"):
                    a = " " + a
                safe.append(a)
            _sp.Popen(
                ["termux-notification"] + safe,
                stdout=_sp.DEVNULL,
                stderr=_sp.DEVNULL,
            )
        except Exception:
    ...
    def _termux_remove_notification(self, panel: str) -> None:
        ...
        try:
            ...
            notif_id = "otrv4_" + self._termux_clean(panel.lstrip("#"), 64)
            _sp.Popen(
    ...
    def _termux_notify_message(self, panel: str, message: str) -> None:
        ...
        args = [
            ...
            _notif_body,
            "--priority",
            "high" if is_encrypted else "default",
            "--id",
            "otrv4_" + panel.lstrip("#"),
            "--alert-once",
        ]
        ...
        if started:
            self._termux_fire(
                [
                    ...
                    "🔒 OTR session started",
                    "--content",
                    "Encrypted session with " + peer + " established",
                    "--priority",
                    "high",
                    "--id",
                    "otrv4_" + peer + "_session",
                    "--vibrate",
    ...
    def _on_peer_disconnected(self, peer: str, reason: str = "") -> None:
        ...
        Idempotent - _disconnected_peers set prevents duplicate messages
        from the 401 flood (one 401 per outbound fragment with no destination).
        """
        if peer not in self.panel_manager.panels:
            return
    ...
    def _on_smp_verified(self, peer: str):
        ...
        self.add_message(
            ...
            colorize(
                "🔵 SMP VERIFIED - identity confirmed by shared secret! (🦀 Rust SMP)", "blue"
            ),
            sec,
        )
        self.add_message(
            "system", f"{colorize ('🔵 SMP verified with','blue')} {colorize_username (peer )}"
        )
        self._termux_fire(
            [
                "--title",
                "\U0001f535 OTR identity verified",
                "--content",
                peer + " - SMP shared secret confirmed",
                "--priority",
                "high",
                "--id",
                "otrv4_" + peer + "_session",
    ...
    def start_guided_otr_session(self, peer: str):
        ...
        if (
            self.session_manager.has_session(peer)
            and self.session_manager.get_security_level(peer) != UIConstants.SecurityLevel.PLAINTEXT
        ):
            if peer not in self.panel_manager.panels:
                self.panel_manager.add_panel(peer, "private")
            self._switch_panel(peer)
            self.add_message(
                peer, colorize(f"✅ Already encrypted with {colorize_username (peer )}", "green")
            )
            return
        ...
        self._switch_panel(peer)
        self.add_message(
            peer, colorize(f"🔑 Starting OTR session with {colorize_username (peer )}…", "cyan")
        )
        ...
        if cmd == "otr" and len(parts) > 1:
            ...
            if remote_fp:
                try:
                    ...
                    self.add_message(
                        peer, colorize("🟢 Fingerprint trusted - VERIFIED", "green"), new_sec
                    )
                except Exception as exc:
            ...
            if subcmd == "start":
                ...
                if (
                    ...
                    self.add_message(
                        "system", f"{colorize ('❌ No encrypted session with','red')} {peer }"
                    )
                    return
                ...
                if not peer and len(parts) > 2:
                    ...
                    self.add_message(
                        "system",
                        colorize(
                            "Usage: /smp <peer> <secret>  or  /smp <secret> (in peer panel)", "red"
                        ),
                    )
                    return
    ...
    def _start_smp(self, peer: str, secret: str, question: str = ""):
        ...
        self.add_message(
            ...
            colorize(
                ...
                " · Computing challenge (ML-KEM-1024 + ML-DSA-87 + ZKP) - please wait…",
                "yellow",
            ),
            sec,
        )
        ...
        def _do_smp():
            try:
                if not hasattr(self.session_manager, "start_smp"):
                    self.add_message(
                        "system", colorize("SMP not supported by session manager", "red")
                    )
                    return
                ...
                if (
                    encrypted_msg
                    and isinstance(encrypted_msg, str)
                    ...
                    if self.send_otr_message(peer, encrypted_msg):
                        ...
                        self.add_message(
                            "system", colorize("❌ Failed to send SMP challenge", "red")
                        )
                        self.panel_manager.update_smp_progress(peer, 0, 0)
                else:
                    self.panel_manager.update_smp_progress(peer, 0, 0)
        ...
        try:
            ...
            if sess:
                if hasattr(sess, "get_fingerprint"):
                    ...
                    if fp:
                        self.debug(
                            f"_get_remote_fp: got from session.get_fingerprint: {fp [:16 ]}..."
                        )
                        return fp
                ...
                if (
                    ...
                    and sess._remote_long_term_pub_bytes
                ):
                    try:
    ...
    def _show_fingerprints(self):
        local_fp = self._get_local_fp()
        self.add_message(
            "system", f"Your fingerprint: {colorize (self ._fmt_fp (local_fp ),'cyan')}"
        )
        ...
        for peer in list(self.session_manager.sessions.keys()):
            ...
            if fp:
                try:
                    is_t = (
                        self.session_manager.is_peer_trusted(peer)
                        if hasattr(self.session_manager, "is_peer_trusted")
                        else self.session_manager.trust_db.is_trusted(peer, fp)
                    )
                    trusted = (
                        colorize(" ✅ trusted", "green")
                        if is_t
                        else colorize(" ⚠ untrusted", "yellow")
                    )
                except Exception:
                    trusted = ""
    ...
    def shutdown(self):
        ...
        _session_count = (
            len(self.session_manager.sessions) if hasattr(self.session_manager, "sessions") else 0
        )
        _rust_count = 0
        ...
        if _rust_count > 0:
            self.add_message(
                "system",
                colorize(
                    f"🦀 {_rust_count } Rust ratchet(s) zeroized (deterministic memory wipe)",
                    "green",
                ),
            )
...
def main():
    ...
    _disp_port = (
        config.port
        if config.port != 0
        else (IRCConstants.TLS_PORT if _net == "clearnet" else IRCConstants.PORT)
    )
    _tls_disp = (
        "🔒 TLS"
        if (_net == "clearnet" and _disp_port == IRCConstants.TLS_PORT) or config.use_tls
        else "plaintext"
    )
    ...
    safe_print(
        f"Debug   : {colorize ('ON'if DEBUG_MODE else 'OFF','green'if DEBUG_MODE else 'dim')}"
    )
    ...
    _dake_label = (
        "🦀 Rust (DH secrets never Python)" if RUST_DAKE_AVAILABLE else "🐍 Python (C extensions)"
    )
    safe_print(f"DAKE    : {colorize (_dake_label ,'green'if RUST_DAKE_AVAILABLE else 'yellow')}")

    _smp_label = (
        "🦀 Rust (ML-KEM-1024 + ML-DSA-87 hybrid PQC, ZeroizeOnDrop)"
        if RUST_RATCHET_AVAILABLE
        else "🐍 Python (C extensions)"
    )
    ...
    if not RUST_RATCHET_AVAILABLE:
        ...
        safe_print(
            colorize("     cd Rust && cargo test --release && maturin build --release", "cyan")
        )
    ...
    if _net_detect == NetworkConstants.NET_I2P:
        ...
        if _sam_ok:
            safe_print(
                f"I2P     : {colorize ('SAM bridge (unique destination per session)','green')}"
            )
        else:
            safe_print(
                f"I2P     : {colorize ('SOCKS5 (shared destination - SAM not available)','yellow')}"
            )
            ...
            try:
                ...
                if not r:
                    ...
                    if not client.running and not client.shutdown_flag and not client._reconnecting:
                        ...
                        while (
                            not client.shutdown_flag
                            and not client.running
                            and not client._reconnecting
                        ):
                            try:
                                ...
                                if l2.startswith("/"):
                                    ...
                                    safe_print(
                                        colorize("Not connected - use /reconnect first.", "yellow")
                                    )
                                _print_prompt()
                ...
                try:
                    ...
                    client.add_message(
                        "system",
                        colorize(f"⚠ Internal error (recovered): {str (exc )[:80 ]}", "yellow"),
                    )
                except Exception:
                    pass
                _print_prompt()
